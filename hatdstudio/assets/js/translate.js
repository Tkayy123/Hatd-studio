...
(function () {
  ...
  var TRANSLATIONS = {
    ...
    'Thoughtful architecture and interior design for homes and businesses across Melbourne. Balancing bold concepts with timeless craftsmanship.':
      'Kiến trúc và thiết kế nội thất tỉ mỉ cho nhà ở và doanh nghiệp khắp Melbourne. Cân bằng giữa ý tưởng táo bạo và sự tinh xảo vượt thời gian.',
  ...
  // ----------------------------------------------------------
  // DEFAULT LANGUAGE = ENGLISH ON EVERY PAGE LOAD
  // ----------------------------------------------------------
  // We intentionally do NOT auto-apply Vietnamese from a saved
  // localStorage preference on load. Doing so raced against the
  // hero typing/slider animation (elements not yet laid out when
  // translate.js read innerText), which occasionally left some
  // lines translated and others not — showing English and
  // Vietnamese mixed together on the same slide.
  //
  // The page now always starts in English. Users can still switch
  // to Vietnamese manually via the toggle button each visit.

  document.addEventListener('DOMContentLoaded', function () {
    // Reset any stale saved preference so every fresh page load is English.
    localStorage.setItem('hatd_lang', 'en');
    currentLang = 'en';
    document.documentElement.lang = 'en';
    var btn = document.getElementById('translateBtn');
    if (btn) btn.innerHTML = '<i class="fas fa-globe"></i> Tiếng Việt';
  });
